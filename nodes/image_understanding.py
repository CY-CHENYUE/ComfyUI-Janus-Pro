...
def Analyze(model, processor, image, question, seed, temperature, top_p, max_new_tokens):
    ...
    prepare_inputs = processor(conversations=conversation, images=[pil_image], force_batchify=True).to(model.device)
...
class JanusImageUnderstanding:
    ...
    def analyze_image(self, model, processor, image, question, seed, temperature, top_p, max_new_tokens):
        ...
        if len(image.shape) == 4:  # BCHW format
            ...
            for i in tensors:
                image = i  # 移除batch维度，现在是 [H, W, C]

                # 确保值范围在[0,1]之间并转换为uint8
                answer = Analyze(model, processor, image, question, seed, temperature, top_p, max_new_tokens)
